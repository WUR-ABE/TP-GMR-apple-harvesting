...
def handle_trajectory_prediction(message):
    ...
    # Predict trajectory
    pred_traj = model.predict([start_end], hor=message.horizon)[0]
...
if __name__ == "__main__":
    rospy.init_node("traj_pred_lqr_server")
    serv = rospy.Service('traj_pred_lqr_service', TrajPred, handle_trajectory_prediction)
    rospy.spin()
