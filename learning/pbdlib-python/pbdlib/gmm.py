...
class GMM(Model):
	...
	def marginal_array(self, dims):
		...
		:return:
		"""
		gmm = GMM(nb_dim=len(dims), nb_states=self.nb_states)
		gmm.priors = self.priors
		gmm.mu = self.mu[:, dims]
		gmm.sigma = self.sigma[:, dims[:,None], dims]
	...
	def marginal_model(self, dims):
		...
		:return:
		"""
		gmm = GMM(nb_dim=dims.start - dims.stop, nb_states=self.nb_states)
		gmm.priors = self.priors
		gmm.mu = self.mu[:, dims]
